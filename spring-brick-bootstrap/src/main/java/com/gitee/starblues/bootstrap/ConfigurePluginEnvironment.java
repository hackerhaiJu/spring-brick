...
public class ConfigurePluginEnvironment {
    ...
    public void configureEnvironment(ConfigurableEnvironment environment, String[] args) {
        ...
        try{
            ...
            this.logger.error("LiveBeansView.registerApplicationContext失败. {}", ex.getMessage(), ex);
        }
    ...
    public void logProfiles(ConfigurableEnvironment environment){
        ...
        if(activeProfiles.length > 0){
            this.logger.info("Plugin[{}] following profiles are active{}: {}",
                    MsgUtils.getPluginUnique(this.pluginDescriptor),
                    fromMainMsg,
                    StringUtils.toStrByArray(activeProfiles));
        } else {
            this.logger.info("Plugin[{}]  No active profile set, falling back to default profiles{}: {}",
                    MsgUtils.getPluginUnique(this.pluginDescriptor),
                    fromMainMsg,
