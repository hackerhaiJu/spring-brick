...
public class PluginStaticResourceResolver extends AbstractResourceResolver {
    ...
    protected Resource resolveResourceInternal(HttpServletRequest request,
        ...
        if(request != null){
            ...
            requestPath = UrlUtils.format(formatUri.replaceFirst(this.config.getPathPrefix(), ""));
        }
        ...
        if (startOffset == -1) {
            ...
            partialPath = this.config.getIndexPageName();
        } else {
        ...
        resource = this.findResource(pluginResource, partialPath);
        if(resource != null){
            ...
            // 第一级节点
            resource = this.findResource(pluginResource, UrlUtils.joiningUrlPath(partialPath, indexPageName));
            if(resource != null){
                return resource;
            }
            // 根节点
            return this.findResource(pluginResource, UrlUtils.joiningUrlPath(UrlUtils.PATH_SEPARATOR, indexPageName));
        }
    }

    private Resource findResource(PluginStaticResource pluginResource, String partialPath){
        // 从classpath 获取资源
        Resource resource = this.resolveClassPath(pluginResource, partialPath);
        if(resource != null){
            return resource;
        }
    ...
    protected String computeKey(HttpServletRequest request, String requestPath) {
        ...
        if (request != null) {
            String codingKey = this.getContentCodingKey(request);
